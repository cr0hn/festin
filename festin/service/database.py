...
class Database:
    ...
    async def _backfill_project_ids(self) -> None:
        ...
        await self._conn.execute(
            "UPDATE domains SET project_id = 1 WHERE project_id IS NULL"
        )
        await self._conn.execute(
    ...
    async def create_domain(
        self, domain_name: str, project_id: int = 1
    ) -> int:
    ...
    async def list_projects(self) -> dict[str, Any]:
        ...
        rows = [
            self._project_counts_row(row)
            for row in await self._fetch_project_aggregates(None)
        ]
    ...
    async def _fetch_project_aggregates(
    ...
    async def list_scans(
        ...
        params: list[Any] = []
        if project_id is not None:
            clauses.append("s.domain_id IN "
                           "(SELECT id FROM domains WHERE project_id = ?)")
        ...
        count_cursor = await self._conn.execute(
            f"SELECT COUNT(*) FROM scans s {where}", params
        )
        total = (await count_cursor.fetchone())[0]
    ...
    async def _get_scan_buckets(self, scan_id: int) -> list[dict[str, Any]]:
        ...
        cursor = await self._conn.execute(
            "SELECT id, name, objects_count FROM buckets "
        ...
        for row in await cursor.fetchall():
            rows.append({
                "id": row[0], "name": row[1], "objects_count": row[2],
            })
        return rows
    ...
    async def persist_scan_results(
        ...
        await self._conn.execute(
            "UPDATE scans SET buckets_found = ?, findings_count = ? "
            "WHERE id = ?",
    ...
    async def _insert_scan_bucket(self, scan_id: int, bucket: Any) -> None:
        ...
        await self._conn.execute(
            "INSERT INTO buckets (scan_id, name, objects_count) "
            "VALUES (?, ?, ?)",
            (
                scan_id,
    ...
    async def get_scan_findings(self, scan_id: int) -> list[dict[str, Any]]:
        ...
        for row in await cursor.fetchall():
            rows.append({
                "id": row[0], "bucket": row[1], "object": row[2],
                "rule": row[3], "severity": row[4], "line": row[5],
                "match": row[6],
            })
        return rows
